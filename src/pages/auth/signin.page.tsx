...
const SignInPage = () => {
    ...
    const handleSubmit = async (e: React.FormEvent) => {
        ...
        setIsLoading(true);
            ...
            axiosConnectionInstance.post(ENDPOINTS.AUTH_TOKEN_OBTAIN, {
                email, password
            }).then(response => {
                const data: { access: string, refresh: string } = response.data
                localStorage.setItem('access_token', data.access)
                localStorage.setItem('refresh_token', data.refresh)
                axiosConnectionInstance.defaults.headers['Authorization'] = 'JWT' + data.access;
                navigate('/')
            }).catch(error => {
    ...
    return (
        <div className="min-h-screen flex items-center justify-center bg-gray-100 px-4 relative">
            ...
            {(isLoading) && <LoaderLayoutComponent />}
        </div>
    );
};
